...
public class AddAndUpdateAssetTypeStepDefinitions {
    ...
    private AssetPlus assetPlus = AssetPlusApplication.getAssetPlus(); // Instance of AssetPlus retrieved from AssetPlusApplication
     ...
     * @param string2 represents lifespan of the assetType
     *                Defines steps so :that manager can successfully add an AssetType
     * @author Namir Habib ; Mahmoud Amin ; Thibaut Chan Teck Su
     */
    ...
    public void the_manager_attempts_to_add_a_new_asset_type_to_the_system_with_name_and_expected_life_span_of_days_p14(
        ...
        error = AssetPlusFeatureSet2Controller.addAssetType(string, parseInt(string2));
    }

    /**
     * @param string  assetType to be updated
     * @param string2 new name of assetType
     * @param string3 new lifespan of assetType
     *                Defines steps so : manager can successfully update an AssetType
     * @author Namir Habib ; Mahmoud Amin ; Thibaut Chan Teck Su
     */
    ...
    public void the_manager_attempts_to_update_an_asset_type_in_the_system_with_name_to_have_name_and_expected_life_span_of_days_p14(String string, String string2, String string3) {
        error = AssetPlusFeatureSet2Controller.updateAssetType(string, string2, parseInt(string3));
    }

    /**
     * @param string represents name of the assetType
     *               Verifies that number of assetTypes in  system is correct
    ...
    public void the_number_of_asset_types_in_the_system_shall_be_p14(String string) {
        assertEquals(assetPlus.numberOfAssetTypes(), parseInt(string));
    }

    /**
     * @param dataTable represents assetTypes that should exist in system
     *                  Verifies that number of assetTypes in datatable exist in assetPLus application
     * @author Sophia Carbone
     */
    @Then("the following asset types shall exist in the system \\(p14)")
    public void the_following_asset_types_shall_exist_in_the_system_p14(io.cucumber.datatable.DataTable dataTable) {
        List<Map<String, String>> rows = dataTable.asMaps();
    ...
    public void the_asset_type_with_name_and_expected_life_span_of_days_shall_exist_in_the_system_p14(String string, String string2) {
        AssetType e = AssetType.getWithName(string); // Get the new element to test

        if (e != null) { // If element exist
            if (assetPlus.getAssetTypes().contains(e)) { // If AssetPlus correctly contains the element
                assertEquals(parseInt(string2), e.getExpectedLifeSpan()); // Then the new expected lifespan should be equal to the expected one
            }
        }
    }
    ...
    public void the_asset_type_with_name_and_expected_life_span_of_days_shall_not_exist_in_the_system_p14(String string, String string2) {
        AssetType e = AssetType.getWithName(string2); // Get the old or new element to test

        if (e != null) { // If element still exist
            if (assetPlus.getAssetTypes().contains(e)) { // If AssetPlus still contains the element
                assertNotEquals(parseInt(string), e.getExpectedLifeSpan()); // Then the new expected lifespan should not be equal to the old one.
            }
        }
    }
