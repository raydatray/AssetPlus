...
public class JsonSerializer {
    ...
    /**
     * Constructs a JsonSerializer instance that can serialize domain model classes
    ...
    private JsonSerializer(String packageName, AbstractDriver serializeDriver) {
        ...
        // XStream blocks (de)serialization of all types for security reasons, except
        // allowed types that match this pattern
     ...
     * human-friendly.
     *
     * For example, "ca.mcgill.ecse.btms.model.MaintenanceTicket" ->
     * "MaintenanceTicket"
     */
    private void setupXStreamAliases(String packageName) {
        ...
        try {
            ClassPath.from(getClass().getClassLoader()).getAllClasses().stream()
                    ...
                    // establish 2-way mapping in XStream between model type name <-> its model
                    // class
    ...
    private static String formatJsonString(final String json) {
        ...
        for (int i = 0, indent = 0; i < chars.length; i++) {
            ...
            if (c == '\"') {
                ...
                continue; // skip the rest of the for loop body and continue execution at the next
                          // iteration
            }
            if (!beginQuotes) {
                result += switch (c) {
                    ...
                    case '}', ']' ->
                        newline + ((indent -= INDENT_WIDTH) > 0 ? String.format("%" + indent + "s", "") : "") + c;
                    case ':' -> c + " ";
